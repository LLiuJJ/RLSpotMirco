...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	if index <= snapshotIndex { // 比第一个日志号还小，直接返回
		DPrintf("{Node %v} rejects replacing log with snapshotIndex %v as current snapshotIndex %v is larger in term %v", rf.me, index, snapshotIndex, rf.currentTerm)
		return
	}
	// 把 (firstIdx ~ index) 区间内日志删除
	rf.logs = shrinkEntriesArray(rf.logs[index-snapshotIndex:])
	rf.logs[0].Command = nil                                      // 第一个操作日志号设为空
	rf.persister.SaveStateAndSnapshot(rf.encodeState(), snapshot) // 存储快照
...
func (rf *Raft) replicateOneRound(peer int) {
	...
	// prevLogIndex 小于当前存储的第一个日志，需要靠快照去追赶
