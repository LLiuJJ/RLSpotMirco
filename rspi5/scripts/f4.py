...
L1 = 12.0  # 大腿长度
L2 = 12.0  # 小腿长度
R = 3.5    # 摆动半径（建议 3.0~4.0，太大会失衡）
center_y = -12.0  # 轨迹圆心 Y（比前进略高，避免拖地）
num_swing = 20    # 摆动相点数
num_support = 20  # 支撑相点数
y_lift_rear = +1.5  # 后腿额外抬高（防拖地）
...
# 生成原地右转轨迹
# ========================
theta_swing = linspace(0, math.pi, num_swing)  # 从 0 到 π

# ---------- 左前腿 (LF)：向前划 ----------
x_swing_LF = [+R * math.cos(t) for t in theta_swing]
y_swing_LF = [center_y + R * math.sin(t) for t in theta_swing]
x_support_LF = [0.0] * num_support
...
# ---------- 右前腿 (RF)：向后划 ----------
x_swing_RF = [-R * math.cos(t) for t in theta_swing]
...
# 初始站立姿势（所有腿在 x=0 支撑）
# ========================
stand_idx = num_swing  # 第20帧：支撑相起始
...
# 原地右转步态主循环
# ========================
steps = 6  # 每 step ≈ 30~45°，6 steps ≈ 180~270°
total_frames = len(hip_LF)  # 应为 40
...
for step in range(steps):
    for i in range(total_frames):
        ...
        time.sleep(0.10)  # 慢速更稳（可调 0.08~0.12）
